...
\begin{frame}[c]
  \titlepage
  \centering
  Available at: \qrcode[hyperlink,height=20mm]{https://speakerdeck.com/aikiriao/ueburetutookimotijiang-zuo}\\ Source: {\scriptsize \myurl{https://github.com/aikiriao/introduction_to_wavelet}}
\end{frame}
...
\begin{frame}[c]
    ...
    \end{figure}
    関数のゼロベクトルを${ }^{\forall} x \ 0(x) := 0$で定める．
\end{frame}
...
\begin{frame}[c]
    ...
    2関数$f,g$の内積を次で定める：
    \begin{align}
        \innprod{f(\cdot)}{g(\cdot)} &:= \int_{a}^{b} f(x) \roverline{g(x)} \mathrm{d}x \label{eq:function_innerprod} \\
        \roverline{g(x)} &: \text{$g(x)$の複素共役} \nonumber
    \end{align}
    積分範囲$[a,b]$は対象により変わる．省略して$\innprod{f}{g}$とも書く．考え方は，有限次元ベクトルと同じ：
    \begin{align*}
...
\begin{frame}[c]
    ...
    \end{align}
    \structure{$f(t)$の$\exp(jn\omega t)$による直交展開}
    \begin{itemize}
...
\begin{frame}[c, allowframebreaks]
    ...
    \begin{enumerate}[(M1)]
        ...
        \item \label{itm:mra2} $V_{m} \subset V_{m+1}$
            \begin{itemize}
                \item[M\ref{itm:mra2}お気持ち] $V_{m}$は入れ子構造をなす．
                    \begin{figure}
                        \includegraphics[width=61mm]{./figs/mra_scaling_set.png}
                    \end{figure}
                    スケール$m$を上げると表現可能な信号が増える
            \end{itemize}
            \newpage
        ...
        \item \label{itm:mra4} $\bigcap_{m} V_{m} = \{ 0 \}$
            \begin{itemize}
                \item[M\ref{itm:mra4}お気持ち] $\displaystyle\lim_{m \to -\infty} V_{m} = \{ 0 \}$であること，つまり，スケールを極限まで落とすと$V_{m}$に含まれる信号は定値関数$0$のみになる
            \end{itemize}
    \end{enumerate}
\end{frame}
...
\begin{frame}[c]
    ...
    \structure{式\eqref{eq:orth_decomp}の展開係数$p_{m}[n], q_{m}[n]$は$h[n], g[n]$を用いて計算できる}．
    \begin{mylemma}[展開計算]
        \vspace*{-15pt}
        \begin{align}
...
\begin{frame}[c]
    ...
    逆に，\structure{$p_{m}[n], q_{m}[n]$から$p_{m+1}[n]$を再構成できる}．
    \begin{mylemma}[再構成計算]
        \vspace*{-15pt}
        \begin{align}
...
\begin{frame}[c]
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[auto, >=latex']
            ...
            \node[dspfilter, right=of input, right=3.0*\dspvspace] (Hm1) {$\roverline{H}$};
            \node[dspnodefull] (Pm1node) at ($(input)!0.5!(Hm1)$) {};
            \node[dspfilter, below=of Pm1node, below=1.0*\dspvspace] (Gm1) {$\roverline{G}$};
            \node[dspfilter, below=of Gm1, below=0.6*\dspvspace] (GDm1) {$\downarrow 2$};
            \node[below=of GDm1, below=1.0*\dspvspace] (Qm) {\small $q_{m}$};
            \node[dspfilter, right=of Hm1, right=0.6*\dspvspace] (HDm1) {$\downarrow 2$};

            \node[dspfilter, right=of HDm1, right=3.0*\dspvspace] (Hm) {$\roverline{H}$};
            \node[dspnodefull] (Pmnode) at ($(HDm1)!0.5!(Hm)$) {};
            \node[dspfilter, below=of Pmnode, below=1.0*\dspvspace] (Gm) {$\roverline{G}$};
            \node[dspfilter, below=of Gm, below=0.6*\dspvspace] (GDm) {$\downarrow 2$};
            \node[below=of GDm, below=1.0*\dspvspace] (Qm_1) {\small $q_{m-1}$};
            \node[dspfilter, right=of Hm, right=0.6*\dspvspace] (HDm) {$\downarrow 2$};

            % \node[dspfilter, right=of HDm, right=3.0*\dspvspace] (Hm_1) {$\roverline{H}$};
            \node[right=of HDm, right=3.0*\dspvspace] (Hm_1) {};
            \node[dspnodefull] (Pm_1node) at ($(HDm)!0.5!(Hm_1)$) {};
            \node[dspfilter, below=of Pm_1node, below=1.0*\dspvspace] (Gm_1) {$\roverline{G}$};
            \node[dspfilter, below=of Gm_1, below=0.6*\dspvspace] (GDm_1) {$\downarrow 2$};
            \node[below=of GDm_1, below=1.0*\dspvspace] (Qm_2) {\small $q_{m-2}$};
            % \node[dspfilter, right=of Hm, right=0.6*\dspvspace] (HDm_1) {$\downarrow 2$};
...
\begin{frame}[c]
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[auto, >=latex']
            ...
            \node[dspfilter, right=of input, right=2.0*\dspvspace] (HIm_1) {$\uparrow 2$};
            \node[dspfilter, right=of HIm_1, right=0.6*\dspvspace] (Hm_1) {$H$};
            \node[dspadder, right=of Hm_1, right=1.0*\dspvspace] (Pmadd) {};
            \node[dspfilter, below=of Pmadd, below=1.0*\dspvspace] (Gm_1) {$G$};
            \node[dspfilter, below=of Gm_1, below=0.6*\dspvspace] (GIm_1) {$\uparrow 2$};
            \node[below=of GIm_1, below=1.0*\dspvspace] (Qm_1) {\small $q_{m-1}$};

            \node[dspfilter, right=of Pmadd, right=2.0*\dspvspace] (HIm) {$\uparrow 2$};
            \node[dspfilter, right=of HIm, right=0.6*\dspvspace] (Hm) {$H$};
            \node[dspadder, right=of Hm, right=1.0*\dspvspace] (Pm1add) {};
            \node[dspfilter, below=of Pm1add, below=1.0*\dspvspace] (Gm) {$G$};
            \node[dspfilter, below=of Gm, below=0.6*\dspvspace] (GIm) {$\uparrow 2$};
            \node[below=of GIm, below=1.0*\dspvspace] (Qm) {\small $q_{m}$};

            \node[right=of Pm1add, right=2.0*\dspvspace] (HIm1) {};
...
\begin{frame}[c]
    \frametitle{他の応用}
    NoteBook\footnote{\myurl{https://github.com/aikiriao/introduction_to_wavelet/blob/main/implementation/fwt_demos.ipynb}}参照．以下のコンテンツを予定．
    \begin{itemize}
        \item 静止画の分解・再構成
        \item ノイズ（欠損）除去
        \item 圧縮（エントロピー観察）
